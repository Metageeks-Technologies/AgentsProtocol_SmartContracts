...
// File @openzeppelin/contracts-upgradeable/proxy/utils/UUPSUpgradeable.sol@v5.2.0
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function batchMint(
        ...
        // Uncomment the payment check if you want to charge for minting
        ...
        emit BatchMinted(msg.sender, startTokenId, _tokenIdCounter - 1);
    }

    function getAllPassHolders() external view returns (address[] memory) {
        return _allHolders;
    }

    function getUserMintedTokens(
        address user
    ) external view returns (MintedToken[] memory) {
        return userMintedTokens[user];
    }

    function hasPriorityPass(address user) external view returns (bool) {
        return priorityPassHolders[user];
    }
